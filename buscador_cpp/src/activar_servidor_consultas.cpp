...
int main(int argc, char* argv[]){
	...
	const char *ruta_log="./log_index.txt";
	//const char *ruta_log="/root/logs/log_index.txt";
	
	if(argc!=5){
		cout<<"\n";
		cout<<"Modo de Uso:\n>./servidor puerto_consulta ruta_indice ruta_grupos binario\n";
	...
	const char *archivo_grupos=argv[3];
	bool binario=atoi(argv[4])>0;
...
//				n = limpiar_consulta(buff_entrada, n, linea);
		...
		if( servidor->lastUpdate() ){
			...
			sprintf(linea, "RESULT %d QUERY %.2f TOTAL %.2f \n", n_res, milis_query, milis_total);
		}
		salida_log.write(linea, strlen(linea));
		salida_log.close();
		linea[0] = 0;
