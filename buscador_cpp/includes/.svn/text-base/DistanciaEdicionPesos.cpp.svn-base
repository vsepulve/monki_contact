...
#include "DistanciaEdicionPesos.h"

inline int DistanciaEdicionPesos::min(int i1, int i2, int i3){
	if(i1<i2){
		if(i1<i3){
			return i1;
		}
...
DistanciaEdicionPesos::DistanciaEdicionPesos(){
	max_palabra=255;
...
DistanciaEdicionPesos::DistanciaEdicionPesos(int _max_palabra){
...
double DistanciaEdicionPesos::d(Dato &d1, Dato &d2){
	//verifricacion de tipos
	if( typeid(d1)!=typeid(DatoTexto) || typeid(d2)!=typeid(DatoTexto) ){
		printf("(DistanciaEdicionPesos::d) - Ambos objetos deben ser DatoTexto\n");
		return DBL_MAX;
	}
	DatoTexto *p1, *p2;
