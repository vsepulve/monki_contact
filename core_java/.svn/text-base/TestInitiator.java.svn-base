...
import obs.grouping.TestProcessingCell;
...
class TestInitiator{
	...
	public static void main(String[]args) throws SQLException {
		...
		if(args.length < 5){
			...
			System.out.println("> java TestInitiator archivo_config contactos_x_celula porcentaje_antiguos num_celulas archivo_output");
			System.out.println("");
			return;
		}
		...
		try{
			...
			System.err.println("TestInitiator - Error al leer configuracion ("+file_config+")");
			e.printStackTrace();
			return;
		}
		Date d_init = null;
		...
		while(true){
			d_init = new Date();
			if(comm.stop()){
				System.out.println("TestInitiator - stop encontrado");
				break;
			}
			System.out.println("\nTestInitiator - Iniciando iteracion " + iteracion);
			...
			System.out.println("TestInitiator - Contactos totales a procesar en iteracion "+iteracion+": "+ totales);
			
			Collection<TestProcessingCell> cells = new ArrayList<TestProcessingCell>();
			Collection<Thread> cellThreads = new ArrayList<Thread>();
			
			for(int h=1; h<=num_cells; ++h){
				Set<Integer> processing = new TreeSet<Integer>();
				
				System.out.println("TestInitiator - Celula " + (h+num_cells*(iteracion-1)) + ": ("+ size_ing/num_cells +" nuevos, "+ size_ed/num_cells +" antiguos)");
				...
				//creo la celula
				TestProcessingCell cell = new TestProcessingCell(host, bd, user, pass, processing);
				cells.add(cell);
			}

			for(TestProcessingCell cell: cells){
			...
			System.out.println("TestInitiator - Iteracion "+iteracion+" terminada ("+tomados_ing.size()+" nuevos, "+tomados_ed.size()+" antiguos, en "+tpo_it+" s)");
			
			if(! comm.open() ){
				System.out.println("TestInitiator - Problemas reabriendo la conexion");
				break;
			}
...
//			}
		...
		System.out.println("TestInitiator - end");
	}
